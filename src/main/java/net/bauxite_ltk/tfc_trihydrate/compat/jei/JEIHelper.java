...
public class JEIHelper implements IModPlugin
{
    ...
    public static IRecipeSlotRichTooltipCallback fluidTooltipCallback = new TFCTHTooltipCallback();
    ...
    private <T extends Recipe<?>> List<T> getFilteredAndSorted(CachedRecipeList<T> cachedList, Predicate<T> include, @Nullable Comparator<T> sorting)
    {
        Stream<T> ret = cachedList.getRecipes(Minecraft.getInstance().level).stream()
                .filter(h -> include.test(h));
        if(sorting!=null)
    ...
    {
        registration.addRecipeCatalyst(TFCTHMultiblockLogic.BALL_MILL.iconStack(), JEIRecipeTypes.BALL_MILL);
