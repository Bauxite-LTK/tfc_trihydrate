...
public class AdvancedVessel implements IMold, ItemContainer, FluidContainer, HeatContainer {
    /**
     ...
     */
    @Nullable
    public static AdvancedVessel get(ItemStack stack)
    {
        final @Nullable IMold mold = IMold.get(stack);
        return mold instanceof AdvancedVessel vessel ? vessel : null;
    }

    private final ItemStack stack;
    private final AdvancedVessel.ContainerInfo containerInfo;
    private final IHeat heat;

    private AdvancedVesselComponent vessel;

    public AdvancedVessel(ItemStack stack, AdvancedVessel.ContainerInfo containerInfo)
    {
        this.stack = stack;
        this.containerInfo = containerInfo;
        this.vessel = Objects.requireNonNull(stack.get(TFCTHComponents.ADVANCED_VESSEL));
    ...
    public int fill(FluidStack input, FluidAction action)
    {
        ...
        else
        {
            ...
            stack.set(TFCTHComponents.ADVANCED_VESSEL, vessel = vessel.with(newContent));
            return result;
        }
    }
    ...
    {
        @Nullable AdvancedVesselComponent updated = null;
        for (int slot = 0; slot < AdvancedVesselComponent.SLOTS; slot++)
        {
    ...
    private void updateWith(AdvancedVesselComponent vessel)
    {
        stack.set(TFCTHComponents.ADVANCED_VESSEL, this.vessel = vessel);
        updateHeatCapacity();
    }
